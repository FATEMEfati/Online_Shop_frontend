async function handleLogin(event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const user_name_response = await fetch('http://127.0.0.1:8000/api-v1/token_info/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            document.getElementById('message').innerText = 'Error: ' + response.statusText + " "+ "Invalid username or password";
        }
    } catch (error) {
...
async function isLoggedIn() {
    ...
    try {
        const response = await fetch('http://127.0.0.1:8000/api-v1/token_info/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
...
function register(event) {
    ...
    fetch('http://127.0.0.1:8000/api-v1/register_users/', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function handleotp(event) {
    ...
    const url = "http://127.0.0.1:8000/api-v1/generate-code/"; 
...
async function send_code(event) {
    ...
    const url = "http://127.0.0.1:8000/api-v1/validate-code/"; 
    ...
    try {
        ...
        if (response.ok) {
            ...
            const user_name = await fetch('http://127.0.0.1:8000/api-v1/token_info/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
...
async function user_info() {
    ...
    try {
        ...
        const user_info_response = await fetch(`http://127.0.0.1:8000/api-v1/user_info/${userId}`
            , {
...
function createForm(userInfoData) {
    ...
    <form id="form_info">
        ...
        <input type="text" id="username" class="login_input" name="username" value="${userInfoData[0].username || ''}" required><br>

        <label for="first_name">First Name:</label>
        <input type="text" class="login_input" id="first_name" name="first_name" value="${userInfoData[0].first_name || ''}" required><br>

        <label for="last_name">Last Name:</label>
        <input type="text" class="login_input" id="last_name" name="last_name" value="${userInfoData[0].last_name || ''}" required><br>

        <label for="gender">Gender:</label>
        <select id="gender" class="login_input" name="gender" required>
            <option value="" disabled ${!userInfoData[0].gender ? 'selected' : ''}>Select Gender</option>
            <option value="m" ${userInfoData[0].gender === 'male' ? 'selected' : ''}>Male</option>
            <option value="f" ${userInfoData[0].gender === 'female' ? 'selected' : ''}>Female</option>
            <option value="u" ${userInfoData[0].gender === 'uncertain' ? 'selected' : ''}>uncertain</option>
        </select><br>

        <label for="date_of_birth">Date of Birth:</label>
        <input type="date" class="login_input" id="date_of_birth" name="date_of_birth" value="${userInfoData[0].date_of_birth || ''}" required><br>

        <label for="email">Email:</label>
        <input type="email" class="login_input" id="email" name="email" value="${userInfoData[0].email || ''}" required><br>

        <label for="mobile_number">Mobile Number:</label>
        <input type="tel" class="login_input" id="mobile_number" name="mobile_number" value="${userInfoData[0].phone_number || ''}" required><br>

        <button type="submit" class="login_button">Submit</button>
    </form>
    <h6>point:${userInfoData[0].point}</h6>
    <button  class="login_button" onclick="user_change_password(${userInfoData[0].id})">change your password</button>
    `;
...
async function submit_user_info(formData, userId) {
    console.log(JSON.stringify(formData));
    try {
        const updateResponse = await fetch(`http://127.0.0.1:8000/api-v1/update_user_info/${userId}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('jwtToken')}`
...
async function change_pass_backend(event) {
    ...
    console.log(JSON.stringify(formData))
    fetch(`http://127.0.0.1:8000/api-v1/update_user_pass/${user_id}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('jwtToken')}`
...
async function show_orders() {
    ...
    try {
        ...
        const orderDataResponse = await fetch(`http://127.0.0.1:8000/api-v1/orders/${userId}`
            , {
                method: 'GET', // Assuming you use PUT or PATCH for updates
                headers: {
                    'Content-Type': 'application/json',
...
async function renderOrderDetailData(item) {
    const orderDetailContainer = document.getElementById('order_detail_container');
    ...
    if (item.pay_status === 'progressing') {
        paymentButtonHTML = `<button class="login_button" onclick="pay_order(${JSON.stringify(item)})">Pay</button>`;
    }
...
async function orderItems(item) {
    ...
    try {
        ...
        const orderDataResponse = await fetch(`http://127.0.0.1:8000/api-v1/orderItem/${item}`, {
            method: 'GET', // Assuming you use PUT or PATCH for updates
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('jwtToken')}`
...
async function renderOrderItemDetailData(item) {
    const orderDetailContainer = document.getElementById('orderitem_detail_container');
...
async function delete_order(item) {
    try {
        const updateResponse = await fetch(`http://127.0.0.1:8000/api-v1/delete_order/${item}`,
                {
                method: 'GET', // Assuming you use PUT or PATCH for updates
                headers: {
                    'Content-Type': 'application/json',
...
async function show_address() {
    ...
    try {
        ...
        const addressDataResponse = await fetch(`http://127.0.0.1:8000/api-v1/address_for_user/${userId}`
            , {
                method: 'GET', 
                headers: {
                    'Content-Type': 'application/json',
        ...
        if (!addressDataResponse.ok) {
            throw new Error(`Address fetch failed: ${addressDataResponse.statusText}`);
        }
...
async function renderAddressDetailData(item) {
    const addressDetailContainer = document.getElementById('address_detail_container');
...
async function submit_address_info(formData, addressId) {
    try {
        const updateResponse = await fetch(`http://127.0.0.1:8000/api-v1/address_for_user/${formData.user}`, {
...
function createAddressForm(item) {
    ...
    <form id="address_info">
        ...
        <input type="text" id="city" class="login_input" name="city" value="${item.city || ''}" required><br>
        <label for="street_name">Street Name:</label>
        <input type="text" class="login_input" id="street_name" name="street_name" value="${item.street_name || ''}" required><br>
        <label for="postal_code">Postal Code:</label>
        <input type="text" class="login_input" id="postal_code" name="postal_code" value="${item.postal_code || ''}" required><br>
        <label for="description">Description:</label>
        <input type="text" class="login_input" id="description" name="description" value="${item.description || ''}" required><br>
        <button type="submit" class="login_button">Submit</button>
    </form>
    `;
}
...
async function add_address_sub(event) {
    ...
    try {
        ...
        const addressResponse = await fetch('http://127.0.0.1:8000/api-v1/create_address_for_user/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
...
async function delete_address(item) {
    ...
    try {
        const updateResponse = await fetch(`http://127.0.0.1:8000/api-v1/address_for_user/${item.user}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('jwtToken')}`
