...
async function updateNumber() {
  const number = await fetch(
    "http://194.5.193.46:8000/api-v1/number_of_product/",
    {
  ...
  const dynamicNumberElement_login = document.getElementById(
    "dynamic-number_login"
  );
...
async function add_to_cart(event) {
  ...
  const apiUrl = "http://194.5.193.46:8000/api-v1/add_to_cart/";
...
async function sendProductData(productId) {
  ...
  const apiBaseUrl = "http://194.5.193.46:8000/api-v1/";
  const product_detailData = {};

  try {
    const productResponse = await fetch(
      `${apiBaseUrl}product_details/${productId}/`
    );
    ...
    const product_key_value_Response = await fetch(
      `${apiBaseUrl}product_attribute_details/${productId}/`
    );
    if (!product_key_value_Response.ok) {
      throw new Error(
        `Product fetch failed: ${product_key_value_Response.statusText}`
      );
    }
    ...
    const productAttributeResponse = await fetch(
      `${apiBaseUrl}product_diversity_details/${productId}/`
    );
    if (!productAttributeResponse.ok) {
      throw new Error(
        `Product fetch failed: ${productAttributeResponse.statusText}`
      );
    }
        ...
        <div class="container pb-5">
            <div class="row">
                <div class="col-lg-5 mt-5">
                    <div class="card mb-3">
                        ${product_detailData.photos
                          .map(
                            (photo) => `
                            ${
                              photo.product ===
                                product_detailData.product_details[0].id &&
                              photo.banner
                                ? `
                                <img class="card-img img-fluid product_picture" src="${
                                  photo.picture.startsWith("http")
                                    ? photo.picture
                ...
                <div class="col-lg-7 mt-5">
                    <div class="card">
                        <div class="card-body">
                            <h1 class="h2">${
                              product_detailData.product_details[0].product_name
                            }</h1>
                            <p class="h3 py-2">
                                ${
                                  product_detailData.product_details[0]
                                    .is_discount
                            ...
                            <h6>Description:</h6>
                            <p>${
                              product_detailData.product_details[0].description
                            }</p>
                            <h6>Quality:</h6>
                            ${product_detailData.product_key_value
                              .map(
                                (key_value) => `
...
async function sendCategoryData(categoryId) {
  ...
  try {
    ...
    const productResponse = await fetch(
      `${apiBaseUrl}product_for_cat/${categoryId}/`
    );
    if (!productResponse.ok) {
      throw new Error(`Product fetch failed: ${productResponse.statusText}`);
    }
                            ...
                            <div class="card rounded-0">
                                      ...
                                      photo.banner
                                        ...
                                        <img src="${
                                          photo.picture.startsWith("http")
                                            ? photo.picture
                                            : "/static" + photo.picture
                                        }" class="card-img-top product_picture" alt="..."> 
                                    `
                                        : ""
                                    }
                                `
                                ...
                                <div class="card-img-overlay rounded-0 product-overlay d-flex align-items-center justify-content-center">
                                    <ul class="list-unstyled">
                                        <button onclick="sendProductData(${
                                          product.id
                                        })" class="btn btn-success text-white mt-2">Show Details</button>
                                    </ul>
                                </div>
                            </div>
                            <div class="card-body">
...
async function discountData() {
  ...
  try {
    const response = await fetch(
      "http://194.5.193.46:8000/api-v1/products/?is_discount=true"
    );
    ...
    const photosResponse = await fetch(
      `http://194.5.193.46:8000/api-v1/galery/`
    );
    if (!photosResponse.ok) {
      throw new Error(`Photo fetch failed: ${photosResponse.statusText}`);
    }
        ...
        <div class="container py-5">
            ...
            <div class="row">
                ${featuredProducts
                  .map((pro) => {
                    ...
                    const productPhotos = discountdata.photos.filter(
                      (photo) => photo.product === pro.id && photo.banner
                    );
                    const productImage =
                      productPhotos.length > 0
                        ? productPhotos[0].picture
                        : "placeholder.jpg";
                    ...
                    return `
                        <div class="col-md-4">
                            <div class="card mb-4 product-wap rounded-0">
                                <div class="card rounded-0">
                                    <img src="${productImage}" class="card-img-top product_picture" alt="${
                      pro.product_name
                    }"> 
                                    <div class="card-img-overlay rounded-0 product-overlay d-flex align-items-center justify-content-center">
                                        <button onclick="sendProductData(${
            ...
            <div class="row">
                <div class="col text-center">
                    <button onclick="changePage_discount(-1)" class="btn btn-primary" ${
                      currentPage === 1 ? "disabled" : ""
                    }>Previous</button>
                    <span class="mx-2">Page ${currentPage} of ${totalPages}</span>
                    <button onclick="changePage_discount(1)" class="btn btn-primary" ${
...
async function sendMostPData() {
  ...
  try {
    const response = await fetch(
      "http://194.5.193.46:8000/api-v1/top_product/?limit=25"
    );
        ...
        <div class="container py-5">
            ...
            <div class="row">
                ${featuredProducts
                  .map((pro) => {
                    // Find the corresponding photo for the product
                    const productPhotos = mostpdata.photos.filter(
                      (photo) =>
                        photo.product === pro.product_id && photo.banner
                    );
                    const productImage =
                      productPhotos.length > 0
                        ? productPhotos[0].picture
            ...
            <div class="row">
                <div class="col text-center">
                    <button onclick="changePage_mostp(-1)" class="btn btn-primary" ${
                      currentPage === 1 ? "disabled" : ""
                    }>Previous</button>
                    <span class="mx-2">Page ${currentPage} of ${totalPages}</span>
                    <button onclick="changePage_mostp(1)" class="btn btn-primary" ${
...
async function performSearch() {
  ...
  try {
    const response = await fetch(
      `http://194.5.193.46:8000/api-v1/search?q=${encodeURIComponent(query)}`
    );
...
function displayResults(data) {
  ...
  productSection.innerHTML = `
        <div class="container py-5">
            ...
            <div class="row">
                <div class="col-12 text-center">
                    <button onclick="changePage_search(-1)" class="btn btn-primary" ${
                      currentPage === 1 ? "disabled" : ""
                    }>Previous</button>
                    <span class="mx-2">Page ${currentPage} of ${totalPages}</span>
                    <button onclick="changePage_search(1)" class="btn btn-primary" ${
  ...
  try {
    ...
    for (const item of cart_data) {
      ...
      if (!product_response.ok) {
        throw new Error(
          `Product fetch failed for ID ${productId}: ${product_response.statusText}`
        );
      }
      try {
                              ...
                              .map(
                                ...
                                <img class="card-img img-fluid" src="${
                                  photo.picture.startsWith("http")
                                    ? photo.picture
                                    : "/static" + photo.picture
                                }" id="product-detail" style="width: 345px; height: auto;">
                                `
                                        : ""
                                    }
                                `
                        ...
                        <div class="col-lg-7 mt-5">
                            <div class="card">
                                ...
                                <ul class="list-inline pb-3">
                                        <li class="list-inline-item text-right">Quantity</li>
                                        <button type="button" class="list-inline-item no-box-button">
                                        <span class="btn btn-success" id="btn-minus" onclick="decreaseQuantity_cart(${
                                          product_detailData.product_details[0]
                                            .id
                                        },${product_detailData.size},'${
    product_detailData.color
  }')">-</span>
                                        </button>
                                        <li class="list-inline-item">
                                        ...
                                        <button type="button" class="list-inline-item no-box-button">
                                        <span class="btn btn-success" id="btn-plus" onclick="increaseQuantity_cart(${
                                          product_detailData.product_details[0]
                                            .id
                                        },${product_detailData.size},'${
    product_detailData.color
                                        ...
                                        <div class="row pb-3">
                                            <div class="col d-grid">
                                                <button  class="btn btn-success btn-lg" name="submit" onclick = "sendProductData(${
                                                  product_detailData
                                                    .product_details[0].id
                                                })">show details</button>
                                            </div>
                                            <div class="col d-grid">
                                                <button  class="btn btn-success btn-lg" name="submit" onclick = "delete_from_cart(${
                                                  product_detailData
                                                    .product_details[0].id
                                                },${product_detailData.size},'${
    product_detailData.color
...
function decreaseQuantity_cart(product_id, size, color) {
  ...
  if (currentValue > 1) {
    const apiUrl = "http://194.5.193.46:8000/api-v1/remove_from_cart/";
    const data = {
      product_id: product_id,
      quantity: 1,
      size: size,
...
function delete_from_cart(product_id, size, color) {
  ...
  const apiUrl = "http://194.5.193.46:8000/api-v1/delete_from_cart/";
...
async function total_price() {
  ...
  try {
    const total_price_response = await fetch(
      "http://194.5.193.46:8000/api-v1/total_price/",
      {
        method: "GET",
        credentials: "include",
        headers: {
                ...
                <form id="orderForm1" onsubmit="sendorderdata(event)">
                    ...
                    <select class="login_input" id="address" name="address">
                        ${addresses
                          .map(
                            (addr) =>
                              `<option value="${addr.id}">${addr.postal_code}</option>`
                          )
                          .join("")}
                    </select>
                    <br/>
...
async function sendorderdata(event) {
  ...
  fetch("http://194.5.193.46:8000/api-v1/create_order/", {
    method: "POST",
    credentials: "include",
    headers: {
      Authorization: `Bearer ${token}`,
